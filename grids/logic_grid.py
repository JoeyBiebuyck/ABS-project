...
class Centralised_grid(Grid):
    def __init__(self, item_to_pos_dict, size, choose_strategy=util.random_action, move_strategy=util.astar, nr_of_agents=2, agent_capacity=2, cell_size=30, nr_of_centralised_agents=1, record_stats=False):
        super().__init__(item_to_pos_dict, size, cell_size=cell_size, record_stats=record_stats)
    ...
    def play(self):
        while self.running:
            for central_agent in self.central_agents:
                central_agent.play()
                #self.grid_ui.update_ui(self.logic_grid)  TODO uncomment voor visual grid
                #time.sleep(0.1)
        if not self.running:  # Geeft de statistieken van elke agent terug als alle bestellingen behandeld zijn
            for agent in self.working_agents:
                if self.record_stats:
